<template>
    <!-- Main jumbotron for a primary marketing message or call to action
    -->
    <div class="container">
      <!-- Example row of columns -->
      <div class="row padtop">
        <div class="col-md-12">
          <h1>海丁的號角</h1>
          <div class="hrn1"></div>
        </div>
        <div class="col-md-8">
          <div class="n1">
            <p>海丁的號角為沙拉丁戰役的第三關劇情關卡，此關為第三次十字軍東征哈丁戰役(今哈丁角，太巴列，巴勒斯坦)的原型，十字軍作戰攜帶的聖物真十字架落入穆斯林手裡，嚴重打擊十字軍軍隊的士氣。</p>
          </div>
          <h2>玩家</h2>
          <div class="hrn1"></div>
          <div class="n1">
            <h3>基本資訊</h3>
            <table class="table-bordered" width="100%">
              <tr>
                <td class="tc1" width="15%">起始時代:</td>
                <td class="tc2" width="15%">封建時代</td>
                <td class="tc1" width="15%">起始資源:</td>
                <td class="tc2">600木頭，400食物，600黃金，400石頭</td>
                <td class="tc1" width="15%">人口上限:</td>
                <td class="tc2" width="5%">75</td>
              </tr>
            </table>
            <h3>AI</h3>
            <!--<span class="glyphicon glyphicon-star" aria-hidden="true" style="color:
            #ff911d; margin-left: 5px;"></span>:中立</p>!-->
            <table class="table-bordered" width="100%">
              <tr>
                <td class="tc4" colspan="4" style="background: #2980b9;">玩家本身</td>
              </tr>
              <tr>
                <td class="tc6">
                  <div class="p3">玩家1：</div></td>
                <td class="tc6" colspan="3">薩拉森人（綠色）</td>
              </tr>
              <tr>
                <td class="tc4" colspan="4" style="background:#ea6153;">敵對</td>
              </tr>
              <tr>
                <td class="tc6">
                  <div class="p2">玩家2：</div></td>
                <td class="tc6">英國 （紅色）不列顛</td>
                <td class="tc6">
                  <div class="p1">玩家3：</div></td>
                <td class="tc6">耶路撒冷（藍色）拜占庭</td>
              </tr>
              <tr>
                <td class="tc6">
                  <div class="p4">玩家4：</div></td>
                <td class="tc6">聖殿騎士（黃色）條頓 敵對</td>
                <td class="tc6">
                  <div class="p8">玩家5：</div></td>
                <td class="tc6">救傷團（橘色）條頓</td>
              </tr>
            </table>
            <br/>
            <dl class="dl-horizontal"> <dt>開場：</dt> 
              <dd>12位村民、14位長槍兵、6位矛兵、4位馬騎弓兵、1台輕型投石車。</dd> <dt>大地之母單位：</dt> 
              <dd>無</dd>
            </dl>
             <h2>主要任務</h2>
            <div class="hrn1"></div>
            <div class="n1">
              <ol>
                <li>奪取真十字架的碎片(遺跡)並把它運回海丁角(有旗子作標記)。</li>
              </ol>
            </div>
          </div>
          <h2>任務提示</h2>
          <div class="hrn1"></div>
          <div class="n1">
            <ul>
              <li>在這個事件裡，最好防衛就是要作出最好的攻擊。在底比里亞斯湖裡並沒有太多的魚，只有以農業來支撐你的經濟。
                <li>就像得到其他遺跡一樣，你的敵軍會把真十字架的碎片守衛在修道院裡面。小心聖殿聖騎和救傷團-他們是你最危險的對手。</li>
                <li>注意：在沙漠當中連小石頭都很少見得到。所以你只能依靠強大的軍隊，而不是城牆或箭塔。</li>
            </ul>
          </div>
        </div>
        <div class="col-md-4">
          <table class="table-bordered" width="100%">
            <tr>
              <td class="unit" colspan="2" style=" background-color:#375d96;">
                <div class="unititle"><nuxt-link to="">海丁的號角</nuxt-link>
                </div>
              </td>
            </tr>
            <tr>
              <td colspan="2">
                <div class="unititle">
                  <img src="~assets/Horns_of_hattin.png" width="" alt="">
                </div>
              </td>
            </tr>
            <tr>
              <td class="unit" colspan="2" style=" background-color:#375d96;">
                <div class="unititle2"><nuxt-link to="">情景信息</nuxt-link>
                </div>
              </td>
            </tr>
            <tr>
              <td class="unit3">版本</td>
              <td class="unit4">帝王世紀</td>
            </tr>
            <tr>
              <td class="unit3">戰役</td>
              <td class="unit4">沙拉丁</td>
            </tr>
            <tr>
              <td class="unit3">文明</td>
              <td class="unit4"><nuxt-link to="">薩拉森人</nuxt-link>
              </td>
            </tr>
<tr><td class="unit" colspan="2" style=" background-color:#375d96;">
              <div class="unititle2"><nuxt-link to="">戰役關卡編號</nuxt-link>
              </div>
            </td>
            </tr>
            <tr>
              <td class="unit3">章節</td>
              <td class="unit4">帝王世紀第三章之三</td>
            </tr>
            <tr>
              <td class="unit3">上一關</td>
              <td class="unit4"> <nuxt-link to="/batt/The_Horns_of_Hattin">阿拉伯之主</nuxt-link> 
            </tr>
            <tr>
              <td class="unit3">下一關</td>
              <td class="unit4"> <nuxt-link to="/batt/The_Siege_of_Jerusalem">耶路撒冷攻城戰</nuxt-link> 
              </td>
            </tr>
          </table>
          <div class="thanks">感謝! 此頁面由裕暉，亞諭協助編輯</div>
        </div>
      </div>
      <div class="row">
        <div class="col-md-12"></div>
      </div>
    </div>
    
</template>
<script>
export default {
  layout: 'allbatt',
  components: {
  }
}
</script>
<style>
</style>