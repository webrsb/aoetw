<template>
  <!-- Main jumbotron for a primary marketing message or call to action
    -->
  <div class="container">
    <!-- Example row of columns -->
    <div class="row padtop">
      <div class="col-md-12">
        <h1>異端邪說</h1>
        <div class="hrn1"></div>
      </div>
      <div class="col-md-8">
        <div class="n1">
          <p>
            異端邪說可於<a href="Castle_Age.php">城堡時代</a>在<a
              href="Monastery.php"
              >修道院</a
            >
            研發後可讓被敵方 <a href="Monk.php">僧侶</a>和<a
              href="Missionary.php"
              >傳教士</a
            >招降的單位死亡，建築則直接毀壞。如果對手出了很多僧侶，那麼異端就必須被研發，即使這個科技非常的昂貴。
          </p>
        </div>
        <h3>文明可研發與否</h3>
        <div class="hrn1"></div>
        <div class="row">
          <div class="col-md-12">
            <div class="n1">
              <h4>
                <strong
                  ><span
                    class="glyphicon glyphicon-ok okc"
                    aria-hidden="true"
                  ></span
                  ><span class="" style="margin-left: 2px;"
                    >文明可研發</span
                  ></strong
                >
              </h4>
              <a href="Aztecs.php">阿茲特克</a>、
              <a href="Berbers.php">柏柏</a>、
              <a href="Byzantines.php">拜占庭</a>、
              <a href="Celts.php">賽爾特</a>、
              <a href="Ethiopians.php">衣索比亞</a>、
              <a href="Franks.php">法蘭克</a>、 <a href="Huns.php">匈奴</a>、
              <a href="Incas.php">印加</a>、 <a href="Magyars.php">馬扎爾</a>、
              <a href="Malay.php">馬來</a>、 <a href="Malians.php">馬利</a>、
              <a href="Mayans.php">馬雅</a>、 <a href="Mongols.php">蒙古</a>、
              <a href="Portuguese.php">葡萄牙</a>、
              <a href="Saracens.php">薩拉森</a>、
              <a href="Spanish.php">西班牙</a>、
              <a href="Teutons.php">條頓</a>、 <a href="Turks.php">土耳其</a>、
              <a href="Vikings.php">維京</a>。
            </div>
          </div>
        </div>
        <div class="row">
          <div class="col-md-12">
            <div class="n1">
              <h4>
                <strong
                  ><span
                    class="glyphicon glyphicon-remove noc"
                    aria-hidden="true"
                  ></span
                  ><span class="" style="margin-left: 2px;"
                    >文明不可研發</span
                  ></strong
                >
              </h4>
              <a href="Britons.php">不列顛</a>、
              <a href="Burmese.php">緬甸</a>、 <a href="Chinese.php">中國</a>、
              <a href="Goths.php">哥德</a>、 <a href="Indians.php">印度</a>、
              <a href="Italians.php">義大利</a>、
              <a href="Japanese.php">日本</a>、 <a href="Khmer.php">高棉</a>、
              <a href="Koreans.php">韓國</a>、 <a href="Persians.php">波斯</a>、
              <a href="Slavs.php">斯拉夫</a>、
              <a href="Vietnamese.php">越南</a>。
            </div>
          </div>
        </div>
        <h3>文明加成</h3>
        <div class="hrn1"></div>
        <div class="n1">
          <ul>
            <li><a href="Aztecs.php">阿茲特克</a>：研發該科技給與僧侶+5HP。</li>
          </ul>
        </div>
        <h3>歷史</h3>
        <div class="hrn1"></div>
        <div class="n1">
          異端指非正統信仰之間，稱呼外來的宗教為「異教」，而那些異類教徒大多泛指多為不同信仰的非主流人士，極為少數人一派。異端常常被主流人士排擠，他們大多是被主流教者為了保存宗教主旨的教義純潔性，與正當性，優越且完整性，正統的繼承者，而往往推向谷底深淵，不信教者，不同教者是往往是被排他主義者拿來做攻擊、揶揄、投射、等肅清抹滅的對象。
        </div>
        <h3>趣事</h3>
        <div class="hrn1"></div>
        <div class="n1">
          <ul>
            <li>
              <a href="Italians.php">義大利</a
              >沒有異端，因為他們一直以來都是純正羅馬天主教的中心。
            </li>
          </ul>
        </div>
      </div>
      <div class="col-md-4">
        <table class="table-bordered" width="100%">
          <tr>
            <td class="unit" colspan="2" style=" background-color:#31b0d5;">
              <div class="unititle"><a href="">異端邪說</a></div>
            </td>
          </tr>
          <tr>
            <td colspan="2">
              <div class="unititle">
                <img src="~assets/Heresy.png" width="45" alt="" />
              </div>
            </td>
          </tr>
          <tr>
            <td class="unit" colspan="2" style=" background-color:#31b0d5;">
              <div class="unititle2"><a href="">科技介紹</a></div>
            </td>
          </tr>
          <tr>
            <td class="unit3">文明</td>
            <td class="unit4">請看左方分類</td>
          </tr>
          <tr>
            <td class="unit3">時代</td>
            <td class="unit4">
              <img class="gg" src="~assets/Castle-age-reseach.jpg" alt="" /><a
                href="Castle_Age.php"
                >城堡時代</a
              >
            </td>
          </tr>
          <tr>
            <td class="unit3">研發於</td>
            <td class="unit4">
              <img class="gg" src="~assets/Monasteryicon.png" alt="" /><a
                href="Monastery.php"
                >修道院</a
              >
            </td>
          </tr>
          <tr>
            <td class="unit3">訓練時間</td>
            <td class="unit4">60秒</td>
          </tr>
          <tr>
            <td class="unit" colspan="2" style=" background-color:#31b0d5;">
              <div class="unititle2">成本</div>
            </td>
          </tr>
          <tr>
            <td class="unit3">黃金</td>
            <td class="unit4">1000</td>
          </tr>
          <tr>
            <td class="unit" colspan="2" style=" background-color:#31b0d5;">
              <div class="unititle2">影響</div>
            </td>
          </tr>
          <tr>
            <td class="unit3">影響</td>
            <td class="unit4">被招降的單位直接死亡，建築則直接毀壞。</td>
          </tr>
        </table>

        <div class="thanks">感謝! 此頁面由賜齡協助編輯</div>
      </div>
    </div>
    <div class="row">
      <div class="col-md-12"></div>
    </div>
  </div>
</template>
<script>
export default {
  layout: 'alltech',
  components: {}
}
</script>
<style></style>
