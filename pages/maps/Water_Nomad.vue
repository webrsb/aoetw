<template>
  <div class="container">
    <div class="row padtop">
      <div class="col-md-12">
        <h1>水上游牧</h1>
        <div class="hrn1"></div>
      </div>
      <div class="col-md-8">
        <h3>說明</h3>
        <div class="hrn1"></div>
        <div class="n1">
          <p>
            <b>水上游牧</b>在《<nuxt-link to="/games/aor"
              >世紀帝國II：王者崛起</nuxt-link
            >》版本首次出現。與王者崛起的大多數新地圖一樣，水上游牧是一個“兩棲”地圖，也是最有趣的地圖之一。地圖主要由少部分的陸地，還有許多可建造的淺灘及一些深水洞穴。樹是大片的紅樹林，有許多<nuxt-link
              to="/units/animal/Water_Buffalo"
              >水牛</nuxt-link
            >、活海龜和魚可供採集。玩家開始的配置與<nuxt-link to="/maps/Nomad"
              >游牧</nuxt-link
            >相同，但有更多可用的資源，玩家一樣沒有<nuxt-link
              to="/building/Town_Center"
              >城鎮中心</nuxt-link
            >，<nuxt-link to="/units/Scout_Cavalry">斥候騎兵</nuxt-link
            >，但有7位<nuxt-link to="/units/Villager">村民</nuxt-link
            >與6艘<nuxt-link to="/units/Fishing_Ship">漁船</nuxt-link
            >，比游牧只給3位村民可操作空間更大。玩家要盡可能地蒐集散落的村民跟漁船同時找尋有多重資源的地點安置城鎮中心，通常是金礦或<nuxt-link
              to="/resource/Stone"
              >石礦</nuxt-link
            >較多的地方，漁船除了可以用來捕魚還可以探索未知的地方。
          </p>
          <p>
            如大多數的兩棲地圖同時發展海上及陸上軍隊是勢在必行的，但在水上游牧這張圖更是如此。尤其是<nuxt-link
              to="/units/Demolition_Ship"
              >爆破船</nuxt-link
            >在這張圖有奇效。好的玩家能藉由微操作讓爆破船順利的殲滅一整群敵人。有優秀<nuxt-link
              to="/units/Naval_Vessels"
              >海軍</nuxt-link
            >的<nuxt-link to="/civs">文明</nuxt-link>例如:<nuxt-link
              to="/civs/Malay"
              >馬來</nuxt-link
            >，或是有陸上海上混合優勢的文明例如:<nuxt-link to="/civs/Berbers"
              >柏柏</nuxt-link
            >、<nuxt-link to="/civs/Japanese">日本</nuxt-link
            >在水上游牧都十分好用。
          </p>
        </div>
      </div>
      <div class="col-md-4">
        <img src="~assets/Water_Nomad_-_8_players.png" width="300" alt="" />
        <h4>一個標準水上游牧地圖</h4>
      </div>
    </div>
  </div>
</template>
<script>
definePageMeta({ layout: 'allmap' })

export default {
  components: {}
}
</script>
<style></style>
