<template>
  <div class="container">
    <div class="row padtop">
      <div class="col-md-12">
        <h1>崔斯坦</h1>
        <div class="hrn1"></div>
      </div>
      <div class="col-md-8">
        <div class="n1">
          <p>
            崔斯坦是一位在世紀帝國二：非洲王國資料片中登場的英雄單位，你可在地圖編輯器中找到他。在歷史上，崔斯坦是一名康沃爾人，他是亞瑟王圓桌騎士的成員之一。他最知名的故事是與自己叔父的妻子依所德的一段不倫戀。
          </p>
          <p>
            崔斯坦在遊戲中以馬扎爾驃騎兵的外表登場。所有對騎兵有用的科技與加成對崔斯坦也有效。作為一個英雄單位，他無法被招降且會自動回血。
          </p>
          <p>
            崔斯坦是來自於英國的人物，但製作組卻用馬扎爾驃騎兵而非騎士作為他的外表。沒人知道為什麼。
          </p>
        </div>
      </div>
      <div class="col-md-4">
        <table class="table-bordered" width="100%">
          <tr>
            <td class="unit" colspan="2" style="background-color:#d01022;">
              <div class="unititle2">
                崔斯坦
              </div>
            </td>
          </tr>
          <tr>
            <td colspan="2">
              <div class="unititle">
                <img src="~assets/Magyar_Huszar_icon.jpg" width="80" alt="" />
              </div>
            </td>
          </tr>
          <tr>
            <td class="unit" colspan="2" style="background-color:#d01022;">
              <div class="unititle2" style="color: #fff;">單位介紹</div>
            </td>
          </tr>
          <tr>
            <td class="unit3">類型</td>
            <td class="unit4">騎兵 <br />英雄</td>
          </tr>
          <tr>
            <td class="unit3">出場</td>
            <td class="unit4">非洲王國</td>
          </tr>
          <tr>
            <td class="unit" colspan="2" style="background-color:#d01022;">
              <div class="unititle2" style="color: #fff;">數據</div>
            </td>
          </tr>
          <tr>
            <td class="unit3">血量</td>
            <td class="unit4">140</td>
          </tr>
          <tr>
            <td class="unit3">攻擊</td>
            <td class="unit4">14(近戰)</td>
          </tr>
          <tr>
            <td class="unit3">攻擊加成</td>
            <td class="unit4">+8對攻城器 <br />+2對衝撞車</td>
          </tr>
          <tr>
            <td class="unit3">攻擊速度</td>
            <td class="unit4">1.83</td>
          </tr>
          <tr>
            <td class="unit3">發射速度</td>
            <td class="unit4">5.5</td>
          </tr>
          <tr>
            <td class="unit3">近防</td>
            <td class="unit4">0</td>
          </tr>
          <tr></tr>
          <tr>
            <td class="unit3">遠防</td>
            <td class="unit4">2</td>
          </tr>
          <tr>
            <td class="unit3">防禦類型</td>
            <td class="unit4">騎兵 <br />特殊單位</td>
          </tr>
          <tr>
            <td class="unit3">跑速</td>
            <td class="unit4">1.5</td>
          </tr>
          <tr>
            <td class="unit3">視野</td>
            <td class="unit4">6</td>
          </tr>
        </table>

        <div class="thanks">感謝! 此頁面由Steven協助編輯</div>
        <div class="wiki" style="margin-top: 5px;">
          <div style="float: left;">
            <img src="~assets/Wikipedia-logo.png" width="50" height="46" />
          </div>
          <div style="margin-left: 60px;">
            維基百科上的崔斯坦
            <div style="margin-left: 10px;">
              <i
                ><strong
                  ><a href="https://tinyurl.com/ybrg2gl2">崔斯坦</a></strong
                ></i
              >
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>
<script>
export default {
  layout: 'allhero',
  components: {}
}
</script>
<style></style>
