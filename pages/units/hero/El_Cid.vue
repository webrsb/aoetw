<template>
  <!-- Main jumbotron for a primary marketing message or call to action
    -->
  <div class="container">
    <!-- Example row of columns -->
    <div class="row padtop">
      <div class="col-md-12">
        <h1>熙德</h1>
        <div class="hrn1"></div>
      </div>
      <div class="col-md-8">
        <div class="n1">
          <p>
            <strong>熙德</strong>是《世紀帝國 II HD:
            帝王世紀》中的一名步兵英雄，可以在熙德戰役系列和戰役編輯器中找到。現實中的熙德曾是西班牙貴族和軍事領袖。他在卡斯蒂利亞的桑喬國王下的軍隊服役，之後被放逐到摩爾人的土地，以及他最終的死亡的故事，都在《世紀帝國
            II HD:
            征服者入侵》的熙德戰役有所描述。由於他屬於英雄單位，因此不能被招降且會回血(在征服者資料片後)。熙德在戰役的後段會更名為羅德里高-迪亞茲-熙德並騎者馬，當然他們在戰役中都一定要生存下來。
          </p>
          <p></p>
        </div>
        <h3>圖庫</h3>
        <div class="hrn1"></div>
        <div class="n1">
          <div class="col-md-12">
            <div class="col-md-4">
              <img class="hg" width="200" src="~assets/Elcid.png" alt="" />
              <p><em>戰役編輯器中的熙德(左)和騎馬的熙德。</em></p>
            </div>
            <div class="col-md-5">
              <img
                class="hg"
                width="280"
                src="~assets/Elcidtournament.png"
                alt=""
              />
              <p>
                <em
                  >熙德在戰役"兄弟鬩牆"要在競技場上證明實力以獲得國王的信任。</em
                >
              </p>
            </div>
          </div>
        </div>
      </div>
      <div class="col-md-4">
        <table class="table-bordered" width="100%">
          <tr>
            <td class="unit" colspan="2" style=" background-color:#d01022;">
              <div class="unititle"><nuxt-link to="">熙德</nuxt-link></div>
            </td>
          </tr>
          <tr>
            <td colspan="2">
              <div class="unititle">
                <img src="~assets/Elcidicon.png" width="80" alt="" />
              </div>
            </td>
          </tr>
          <tr>
            <td class="unit" colspan="2" style=" background-color:#d01022;">
              <div class="unititle2" style="color: #fff;">單位介紹</div>
            </td>
          </tr>
          <tr>
            <td class="unit3">類型</td>
            <td class="unit4"><nuxt-link to="">步兵</nuxt-link></td>
          </tr>
          <tr>
            <td class="unit3">出場</td>
            <td class="unit4">帝王世紀</td>
          </tr>
          <tr>
            <td class="unit" colspan="2" style="background-color:#d01022;">
              <div class="unititle2" style="color: #fff;">數據</div>
            </td>
          </tr>
          <tr>
            <td class="unit3">血量</td>
            <td class="unit4">330</td>
          </tr>
          <tr>
            <td class="unit3">攻擊</td>
            <td class="unit4">13(近戰)</td>
          </tr>
          <tr>
            <td class="unit3">攻擊加成</td>
            <td class="unit4">對建築+3</td>
          </tr>
          <tr>
            <td class="unit3">開火速率</td>
            <td class="unit4">2.03</td>
          </tr>
          <tr>
            <td class="unit3">近防</td>
            <td class="unit4">2</td>
          </tr>
          <tr></tr>
          <tr>
            <td class="unit3">遠防</td>
            <td class="unit4">1</td>
          </tr>
          <tr>
            <td class="unit3">防禦類型</td>
            <td class="unit4">步兵</td>
          </tr>
          <tr>
            <td class="unit3">視野</td>
            <td class="unit4">5</td>
          </tr>
        </table>

        <div class="thanks">感謝! 此頁面由賜齡協助編輯</div>
        <div class="row">
          <div class="col-md-12"></div>
        </div>
      </div>
    </div>
  </div>
</template>
<script>
export default {
  layout: 'allhero',
  components: {}
}
</script>
<style></style>
