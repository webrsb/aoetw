<template>
  <div class="container">
    <div class="row padtop">
      <div class="col-md-12">
        <h1>抵達巴里</h1>
        <div class="hrn1"></div>
      </div>
      <div class="col-md-8">
        <div class="n1">
          <p>
            <b>抵達巴里</b>為<nuxt-link to="/batt/Baria">巴里戰役</nuxt-link
            >的第一關劇情關卡，協助東羅馬消滅巴里王朝。
          </p>
        </div>
        <h2>玩家</h2>
        <div class="hrn1"></div>
        <div class="n1">
          <h3>基本資訊</h3>
          <table class="table-bordered" width="100%">
            <tr>
              <td class="tc1" width="14%">起始時代：</td>
              <td class="tc2" width="14%">
                <nuxt-link to="/techs/age/Imperial_Age">帝王時代</nuxt-link>
              </td>
              <td class="tc1" width="14%">起始資源：</td>
              <td class="tc2">無</td>
              <td class="tc1" width="14%">人口上限：</td>
              <td class="tc2" width="5%">0</td>
            </tr>
          </table>

          <p></p>
          <h3>AI</h3>
          <!--<span class="glyphicon glyphicon-star" aria-hidden="true" style="color: #ff911d; margin-left: 5px;"></span>:中立</p>!-->
          <table class="table-bordered" width="100%">
            <tr>
              <td class="tc4" colspan="4" style="background: #2980b9;">
                玩家本身
              </td>
            </tr>
            <tr>
              <td class="tc6"><div class="p1">玩家1</div></td>
              <td class="tc6" colspan="3">
                <nuxt-link to="/civs/Byzantines">拜占庭</nuxt-link>（藍色）
              </td>
            </tr>
            <tr>
              <td class="tc4" colspan="4" style="background: #27ae60;">同盟</td>
            </tr>
            <tr>
              <td class="tc6" width="12%"><div class="p2">玩家2</div></td>
              <td class="tc6" width="38%">
                巴里（紅色）<nuxt-link to="/civs/Saracens">薩拉森</nuxt-link>
              </td>
              <td class="tc6"><div class="p4">玩家3</div></td>
              <td class="tc6">拜占庭帝國（黃色）拜占庭</td>
            </tr>
            <tr>
              <td class="tc6"><div class="p8">玩家5</div></td>
              <td class="tc6">路易士大帝（橘色）法蘭克</td>
              <td class="tc6"><div class="p5">玩家7</div></td>
              <td class="tc6">
                奧里亞（水藍色）<nuxt-link to="/civs/Italians"
                  >義大利</nuxt-link
                >
              </td>
            </tr>
            <tr>
              <td class="tc6"><div class="p7">玩家8</div></td>
              <td class="tc6">
                巴倫第人（灰色）<nuxt-link to="/civs/Goths">哥德</nuxt-link>
              </td>
            </tr>
            <tr>
              <td class="tc4" colspan="4" style="background:#ea6153;">敵對</td>
            </tr>
            <tr></tr>
            <tr>
              <td class="tc6"><div class="p3">玩家4</div></td>
              <td class="tc6">巴里王朝（綠色）薩拉森</td>
              <td class="tc6"><div class="p6">玩家6</div></td>
              <td class="tc6">馬泰拉（紫色）義大利</td>
            </tr>
          </table>

          <br />
          <dl class="dl-horizontal">
            <dt>開場單位：</dt>
            <dd>
              帕尼奧斯.諾蒂卡斯、2位<nuxt-link to="/units/Halberdier"
                >戟兵</nuxt-link
              >。
            </dd>
            <dt>大地之母單位：</dt>
            <dd>
              10位<nuxt-link to="/units/Cataphract">拜占庭聖騎兵</nuxt-link
              >、10位<nuxt-link to="/units/Two-Handed_Swordsman"
                >雙手劍兵</nuxt-link
              >、3位戟兵、5位<nuxt-link to="/units/Arbalest">強弩兵</nuxt-link
              >、5位<nuxt-link to="/units/Heavy_Cavalry_Archer"
                >重裝馬弓騎兵</nuxt-link
              >、1台<nuxt-link to="/units/Siege_Tower">攻城塔</nuxt-link
              >、1台<nuxt-link to="/units/Siege_Ram">重型衝撞車</nuxt-link
              >、10艘<nuxt-link to="/units/Galley">戰船</nuxt-link
              >、5艘<nuxt-link to="/units/Fire_Ship">火戰船</nuxt-link
              >、5艘<nuxt-link to="/units/Elite_Longboat"
                >精銳維京大戰船</nuxt-link
              >。
            </dd>
          </dl>
          <h2>主要任務</h2>
          <div class="hrn1"></div>
          <div class="n1">
            <ol>
              <li>帕尼奧斯必須存活下來。</li>
              <li>
                調出所有在路易士二世營地的拜占庭單位並按編隊部署，因為路易士（橘色<nuxt-link
                  to="/units/Knight"
                  >騎士</nuxt-link
                >）想對單位發表講話。當所有將士集合待命後，點擊路易士二世（橙色的騎士）。
              </li>
              <li>
                帶領信使（只是信使）到馬泰拉的<nuxt-link to="/building/Gate"
                  >城門</nuxt-link
                >開出招降的條件。如果馬泰拉人看見其他的軍隊，他們會認為你在發動進攻。
              </li>
              <li>
                洗劫馬泰拉，摧毀他們除<nuxt-link to="/building/House"
                  >房屋</nuxt-link
                >之外所有的建築。
              </li>
              <li>
                帶帕尼奧斯到路易士那兒，路易士就在馬泰拉毀壞的城門外等待。選擇<nuxt-link
                  to="/units/King"
                  >國王</nuxt-link
                >與他談話。
              </li>
              <li>
                走馬泰拉東邊的路前往奧裡亞。殺死所有城裡的敵軍解放奧裡亞。
              </li>
              <li>
                突襲西南的村莊，殺死你看見的每一個阿拉伯騎兵。搜查整個區域並清理所有道路。一旦你殺死所有人，回去向奧裡亞國王覆命。
              </li>
              <li>
                帶帕尼奧斯回到路易士的營地，向奧裡亞國王覆命以及接受新的任務。
              </li>
              <li>
                航至巴里港口，摧毀所有的<nuxt-link to="/building/Dock"
                  >碼頭</nuxt-link
                >，<nuxt-link to="/units/Naval_Vessels">船艦</nuxt-link
                >和塔，切斷敵人所有的供給。儘量不要損失<nuxt-link
                  to="/units/Transport_Ship"
                  >運輸船</nuxt-link
                >，因為你日後會需要他們。一旦做完，回到路易士那裡。
              </li>
              <li>
                去奧裡亞<nuxt-link to="/building/Castle">城堡</nuxt-link
                >，那裡有工兵在等待你
              </li>
              <li>
                帶工兵去<nuxt-link to="/building/Stone_Wall">城牆</nuxt-link
                >的薄弱點那裡。
              </li>
              <li>
                帕尼奧斯的人破壞了巴里的城牆。巴里人是基督徒所以對你不敵對，但是你必須打敗駐城的阿拉伯軍隊，方能征服巴里(攻佔酋長國巴里)
              </li>
              <li>
                帶路易士去巴里城堡。 注意:
                既然城已淪陷，碼頭港口也任由我們進出了。
              </li>
              <li>帶帕尼奧斯去面見東部拜占庭營地的將軍。</li>
              <li>摧毀鎮守巴里城的五座城堡，讓拜占庭能吞併這座城池。</li>
            </ol>
          </div>
        </div>
        <h2>次要目標</h2>
        <div class="hrn1"></div>
        <div class="n1">
          <ul>
            <li>無</li>
          </ul>
        </div>
        <h2>任務提示</h2>
        <div class="hrn1"></div>
        <div class="n1">
          <ul>
            <li>
              注意：有時，你需要選中另一個單位來開始對話，這種情況下任務會彈出。大多數場景中你將會有一定數量的部隊且不會有更多支援。謹慎地指揮你現有的軍隊，保護好你寶貴的攻城武器。
            </li>
            <li>
              對於任何<nuxt-link to="/batt">戰役</nuxt-link
              >，戰術都是至關重要的。盡可能利用上坡高地的有利優勢和合適的克制兵種。<nuxt-link
                to="/units/Spearman"
                >長槍兵</nuxt-link
              >比<nuxt-link to="/units/Archer_Units">弓箭手</nuxt-link
              >或劍士能更有效的對付<nuxt-link to="/units/Cavalry_Units"
                >騎兵</nuxt-link
              >。聖騎兵是全能的部隊，尤其是<nuxt-link to="/units/Infantry_Units"
                >步兵</nuxt-link
              >的剋星。誘敵戰術對付敵眾我寡的情形也是很有效的。
            </li>
            <li>
              當包圍倫巴第控制的巴里時，把握時機是關鍵。雖然你的圍攻軍隊佔上風，但是如果你等太久倫巴第可能會制服你。
            </li>
            <li>巴里有多個入口，被攻佔的城市不會再有資源重造建築。</li>
            <li>帕尼奧斯.諾蒂卡斯和他的軍隊可以晉級帝王時代。</li>
          </ul>
        </div>
        <h2>斥候</h2>
        <div class="hrn1"></div>
        <div class="n1">
          <ul>
            <li>
              帕尼奧斯在拜占庭某地（黃色）的未名碼頭開始戰役。很快他就會加入前往巴里的拜占庭大艦隊。
            </li>
            <li>
              地圖中心是巴里（紅色），它是主要城市也是你要佔領的目標。巴里由護城河，廣佈的防禦工事，堅固的碼頭包圍，並由許多阿拉伯（綠色）劍士和騎兵守衛。
            </li>
            <li>
              巴里西南方的道路通向敵對城市馬泰拉（粉紅）附近的海灘上路易斯的營地（橘色）。
            </li>
            <li>
              馬泰拉由城牆，城堡，塔，小支守備軍和一支阿拉伯騎兵分遣隊守衛。
            </li>
            <li>巴里西邊和馬泰拉北邊有一片空曠的郊野，那裡匪盜猖獗。</li>
            <li>
              巴里的北邊和村莊的東北方式奧裡亞城（青綠色），那裡被酋長國（綠色）侵佔。他們渴望加入基督教徒布道團。
            </li>
            <li>
              巴里的東北和奧裡亞的東南是拜占庭的宿營（黃色），那裡有一支浩蕩的軍隊。
            </li>
            <li>
              倫巴第（灰色）是最危險的對手。當包圍巴里時，他們會訓練步兵和弓箭手，由<nuxt-link
                to="/units/Monk"
                >僧侶</nuxt-link
              >和<nuxt-link to="/units/Knight">騎士</nuxt-link
              >配合，甚至出動<nuxt-link to="/units/Trebuchet"
                >巨型投石機</nuxt-link
              >。
            </li>
          </ul>
        </div>
        <h2>策略／戰術</h2>
        <div class="hrn1"></div>
        <div class="n1">
          <ul>
            <li>
              你扮演著帕尼奧斯，從拜占庭港口開始，他和兩名士兵一起出發。三人一起搭乘運輸船啟航，直到你獲得幾艘戰艦和運輸船增加部隊。你會遇到巴里酋長艦隊（本節中稱為“阿拉伯人”），擊退對方後，把地面部隊安全地運送到路易二世的營地。
            </li>
            <li>
              皇帝路易二世自我介紹後，你把信使(<nuxt-link
                to="/units/Light_Cavalry"
                >輕騎兵</nuxt-link
              >)送到馬泰拉的大門，輕騎兵會被城堡射死。為了報復他的死亡，你必須擊敗馬泰拉並摧毀除了房屋以外的所有建築物。在你殺死了所有馬泰拉士兵之後，會有一大群阿拉伯軍隊來襲擊你。
            </li>
            <li>
              擊退馬泰拉之後，路易二世會請你入侵奧里亞，殺死所有的阿拉伯軍隊。在遇見奧里亞之王時，他會感謝你，要求你殺死奧里亞和馬泰拉之間剩下的所有阿拉伯軍隊，再返回營地奧裡亞國王覆命以及接受新的任務。
            </li>
            <li>
              路易二世會指派你入侵巴里港口（綠色），摧毀所有的<nuxt-link
                to="/building/Watch_Tower"
                >箭塔</nuxt-link
              >和碼頭，使用<nuxt-link to="/units/Demolition_Ship"
                >爆破船</nuxt-link
              >個別擊破拆掉箭塔，之後在摧毀所有碼頭
            </li>
            <li>
              巴里所有的城門都被鎖定，無法進入，要回到奧里亞找一位工兵，將工兵帶到城外的岩石區，即能消除岩石，路上還是要小心阿拉伯士兵
            </li>
            <li>
              所有部隊潛入巴里，並殺死所有的阿拉伯軍隊。一旦巴里酋長被擊敗，將皇帝路易二世帶到巴里的一座城堡。（在這之前有個過關小技巧，先把紅色巴里同盟改成敵隊，摧毀大部分的建築或城堡，以利最後的通關）
            </li>
            <li>
              你將親眼目睹路易斯二世與貝內文託的倫巴第王子阿德爾奇斯爭吵，並被倫巴第人囚禁。他們將接管巴里和路易二世的營地。你必須在拜占庭<nuxt-link
                to="/building/Barracks"
                >軍營</nuxt-link
              >中與拜占庭指揮官會面，你將接管整個營地和軍隊，並接受貢品，以便隨時補充你的部隊。之後開始摧毀所有的五座城堡圍攻巴里。
            </li>
            <li>敵人資源耗完就沒了，所以可以先防守再進攻。</li>
          </ul>
        </div>
      </div>
      <div class="col-md-4">
        <table class="table-bordered" width="100%">
          <tr>
            <td class="unit" colspan="2" style="background-color:#375d96;">
              <div class="unititle2">
                抵達巴里
              </div>
            </td>
          </tr>
          <tr>
            <td colspan="2">
              <div class="unititle">
                <img src="~assets/Bari_1.jpg" alt="" />
              </div>
            </td>
          </tr>
          <tr>
            <td class="unit" colspan="2" style="background-color:#375d96;">
              <div class="unititle2">
                情景信息
              </div>
            </td>
          </tr>
          <tr>
            <td class="unit3">版本</td>
            <td class="unit4">
              <nuxt-link to="/games/aof">失落的帝國</nuxt-link>
            </td>
          </tr>
          <tr>
            <td class="unit3">戰役</td>
            <td class="unit4"><nuxt-link to="/batt/Baria">巴里</nuxt-link></td>
          </tr>
          <tr>
            <td class="unit3"><nuxt-link to="/civs">文明</nuxt-link></td>
            <td class="unit4">
              <nuxt-link to="/civs/Byzantines">拜占庭人</nuxt-link>
            </td>
          </tr>
          <tr>
            <td class="unit" colspan="2" style="background-color:#375d96;">
              <div class="unititle2">
                戰役關卡編號
              </div>
            </td>
          </tr>
          <tr>
            <td class="unit3">章節</td>
            <td class="unit4">
              <nuxt-link to="/games/aof">失落的帝國</nuxt-link>第三章之一
            </td>
          </tr>
          <tr>
            <td class="unit3">上一關</td>
            <td class="unit4">N/A</td>
          </tr>
          <tr>
            <td class="unit3">下一關</td>
            <td class="unit4">
              <nuxt-link to="/batt/The_Rebellion_of_Melus"
                >邁勒斯叛變</nuxt-link
              >
            </td>
          </tr>
        </table>

        <div class="thanks">感謝！此頁面由裕暉、阿虎協助編輯。</div>
      </div>
    </div>
  </div>
</template>
<script>
export default {
  layout: 'allbatt',
  components: {}
}
</script>
<style></style>
