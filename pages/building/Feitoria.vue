<template>
  <!-- Main jumbotron for a primary marketing message or call to action
    -->
  <div class="container">
    <!-- Example row of columns -->
    <div class="row" style=" margin-top: 50px;">
      <div class="col-md-12">
        <h1>港灣大商站</h1>
        <div class="hrn1"></div>
      </div>
      <div class="col-md-8">
        <div class="n1">
          <p>
            港灣大商站是"世界地國II:非洲王朝"資料片中葡萄牙人進入帝王時代後可以建造的特殊建築物。不需村民蒐幫忙蒐集資源，港灣大商站每秒會給玩家0.8食物、0.8木材、0.45黃金和0.25石頭，也就是每分鐘會給48食物、48木材27黃金和15石頭。不過港灣大商站需要佔用20人口。
          </p>
        </div>
        <h3>戰術</h3>
        <div class="hrn1"></div>
        <div class="row">
          <div class="col-md-12">
            <div class="n1">
              <li>
                除了礙於人口限制之外，玩家可以建造多少個港灣大商站是沒有限制的。
                只要玩家擁有至少一個人口空間，玩家仍然可以建造一座港灣大商站（排隊的單位還是會算入這個空間）。雖然佔據20人口讓蓋港灣大商站看起來似乎不值得，但它給與免費的黃金跟石頭讓人很難評估它的實際價值。
              </li>
              <li>
                港灣大商站最適合黃金和石頭等資源耗盡的後期遊戲。
                玩家必須確保將其放在防守良好的地方，以保護緩慢的資源源源不斷的進入口袋。
              </li>
              <li>
                港灣大商站可以被敵人招降，不過葡萄牙人可以研究異端邪說避免港灣大商站被敵人利用。如果玩家招降了港灣大商站，則人口將被強迫增加20人。
              </li>
            </div>
          </div>
        </div>
        <h3>更多數據</h3>
        <div class="hrn1"></div>
        <div class="n1"></div>
        <table class="table-bordered" width="100%">
          <tr class="tta">
            <td class="unit" colspan="2" style="text-align: center;">
              <div class="unititle">劣勢與優勢</div>
            </td>
          </tr>
          <tr>
            <td class="tta1">強勢(容易對抗)</td>
            <td class="tta4">無。</td>
          </tr>
          <tr>
            <td class="tta1">劣勢(不易對抗)</td>
            <td class="tta4">所有東西。</td>
          </tr>
          <tr class="tta">
            <td class="unit" colspan="2" style="text-align: center;">
              <div class="unititle">升級</div>
            </td>
          </tr>
          <tr>
            <td class="tta1" style="width:20%">生命值</td>
            <td class="tta4">
              <p>
                <img
                  src="~assets/Masonry.jpg"
                  width="30"
                  alt=""
                />磚瓦技術(+10%)
              </p>
              <p>
                <img
                  src="~assets/Architecture.jpg"
                  width="30"
                  alt=""
                />建築學(+10%)
              </p>
            </td>
          </tr>
          <tr>
            <td class="tta1">護甲</td>
            <td class="tta4">
              <p>
                <img
                  src="~assets/Masonry.jpg"
                  width="30"
                  alt=""
                />磚瓦技術(+1/+1，+3建築護甲)
              </p>
              <p>
                <img
                  src="~assets/Architecture.jpg"
                  width="30"
                  alt=""
                />建築學(+1/+1，+3建築護甲)
              </p>
            </td>
          </tr>
          <tr>
            <td class="tta1">視野</td>
            <td class="tta4">
              <p>
                <img
                  src="~assets/Townwatch.jpg"
                  width="30"
                  alt=""
                />城鎮暸望(+4)
              </p>
              <p>
                <img
                  src="~assets/Townpatrol.jpg"
                  width="30"
                  alt=""
                />城鎮巡邏(+4)
              </p>
            </td>
          </tr>
          <tr>
            <td class="tta1" style="width:10%">招降抵抗</td>
            <td class="tta4">
              <p>
                <img src="~assets/Faith.jpg" width="30" alt="" /><a
                  href="Faith.php"
                  >忠誠信仰</a
                >
              </p>
              <p>
                <img src="~assets/Heresy.png" width="30" alt="" /><a
                  href="Heresy.php"
                  >異端邪說</a
                >
              </p>
            </td>
          </tr>
          <tr>
            <td class="tta1">建造速度</td>
            <td class="tta4">
              <p>
                <img
                  src="~assets/Treadmillcrane.jpg"
                  width="30"
                  alt=""
                />滑輪起重機(+20%)
              </p>
            </td>
          </tr>
        </table>

        <h3>團隊加成</h3>
        <div class="hrn1"></div>
        <div class="n1">
          <ul>
            <li>
              <a href="Malians.php">馬利</a
              >：研究磚瓦技術、建築學和踏輪起重機團隊速度提高80%。
            </li>
          </ul>
        </div>
      </div>
      <div class="col-md-4">
        <table class="table-bordered" width="100%">
          <tr>
            <td class="unit" colspan="2" style=" background-color:#3c763d;;">
              <div class="unititle"><a href="">港灣大商站</a></div>
            </td>
          </tr>
          <tr>
            <td colspan="2">
              <div class="unititle">
                <img src="~assets/Feitoriaicon.png" width="45" alt="" />
              </div>
            </td>
          </tr>
          <tr>
            <td class="unit" colspan="2" style=" background-color:#3c763d;;">
              <div class="unititle2"><a href="">概要</a></div>
            </td>
          </tr>
          <tr>
            <td class="unit3">文明</td>
            <td class="unit4">葡萄牙</td>
          </tr>
          <tr>
            <td class="unit3">時代</td>
            <td class="unit4">帝王時代</td>
          </tr>
          <tr>
            <td class="unit3">建築時間</td>
            <td class="unit4">120秒</td>
          </tr>
          <tr>
            <td class="unit3">功能</td>
            <td class="unit4">給予免費資源</td>
          </tr>
          <tr>
            <td class="unit" colspan="2" style=" background-color:#3c763d;;">
              <div class="unititle2"><a href="">成本</a></div>
            </td>
          </tr>
          <tr>
            <td class="unit3">石頭</td>
            <td class="unit4">250</td>
          </tr>
          <tr>
            <td class="unit3">黃金</td>
            <td class="unit4">250</td>
          </tr>
          <tr>
            <td class="unit" colspan="2" style=" background-color:#3c763d;;">
              <div class="unititle2"><a href="">數據</a></div>
            </td>
          </tr>
          <tr>
            <td class="unit3">尺寸</td>
            <td class="unit4">5x5</td>
          </tr>
          <tr>
            <td class="unit3">生命值</td>
            <td class="unit4">5200</td>
          </tr>
          <tr>
            <td class="unit3">近防</td>
            <td class="unit4">3</td>
          </tr>
          <tr>
            <td class="unit3">遠防</td>
            <td class="unit4">10</td>
          </tr>
          <tr>
            <td class="unit3">護甲類別</td>
            <td class="unit4">
              建築 <br />標準建築
              <br />
            </td>
          </tr>
          <tr>
            <td class="unit3">視野</td>
            <td class="unit4">6</td>
          </tr>
        </table>

        <div class="thanks">感謝！本頁面由賜齡協助完成。</div>
      </div>
    </div>
  </div>
</template>
<script>
export default {
  layout: 'allbuilding',
  components: {}
}
</script>
<style></style>
