<template>
  <div class="container">
    <div class="row padtop">
      <div class="col-md-12">
        <h1>老鷹開局</h1>
        <div class="hrn1"></div>
      </div>
      <div class="col-md-8">
        <div class="n1">
          <p>
            老鷹開局為阿拉伯打法之一，為封建出鷹斥侯，而鷹斥侯除了比較怕裝甲步兵外，其弱點較小，過度封建後將給對方極大的壓力。
          </p>
          <ul>
            <li>
              <img
                src="~assets/EagleScoutIcon.jpg"
                width="30px"
                alt=""
              /><nuxt-link to="/units/Eagle_Scout">鷹斥侯</nuxt-link
              >：速度快，不怕槍兵、矛兵、斥侯與少量弓兵，上城堡後威力倍增。
            </li>
          </ul>
        </div>
        <h3>概述</h3>
        <div class="hrn1"></div>
        <div class="n1">
          <p>
            老鷹開局為南美三國的專屬打法，流程為上封建後雙軍營出老鷹，適時的搭配矛兵或槍兵為輔，達到騷擾敵人的效果。
          </p>
          <p>
            且鷹斥侯需求的食物低，在經濟調配上能非常快速的上城堡（甚至可以用市集搶城堡），升級為鷹勇士。
          </p>
        </div>
        <h3>反制</h3>
        <div class="hrn1"></div>
        <div class="n1">
          <div class="row">
            <div class="col-md-12">
              <p>
                在控兵沒有失誤下，老鷹非常的難對付，其打法又很靈活，可以隨意轉矛、槍搭配。
              </p>
              <p>
                但弓兵數量成形後，封建鷹斥侯也不是弓兵的對手。鷹斥侯也非常怕圍牆，只要家裡圍死，鷹斥侯也不能鬧到我方經濟。
              </p>
            </div>
          </div>
          <h3>流程</h3>
          <div class="hrn1"></div>
          <div class="n1">
            <div class="row">
              <div class="col-md-12">
                <em
                  ><p style="padding: 0px 10px;">
                    流程只是大概，沒有一定，每場地形地物都不同，重要的還是找到最適合自己的流程與打法!
                  </p></em
                >
              </div>
              <div class="col-md-6">
                <strong>村民升級流程</strong>
                <ul>
                  <li>2~7p:6人採集羊肉</li>
                  <li>8~11p:4人伐木</li>
                  <li>12p:殺豬</li>
                  <li>13~16p:4人採果</li>
                  <li>17p:殺第二頭豬</li>
                  <li>18～19p:殺豬</li>
                  <li>20～22:豬羊</li>
                  <li>配置：12木4金6果</li>
                </ul>
              </div>
              <div class="col-md-6">
                <strong>升級後流程</strong>
                <ul>
                  <li>點封後起雙軍營</li>
                  <li>出老鷹防守並適時的打回去</li>
                  <li>視戰況研發兵工廠或輔矛兵或槍兵</li>
                  <li>盡可能早點上城堡，發揮老鷹的優勢</li>
                </ul>
              </div>
            </div>
            <h3>適合文明</h3>
            <div class="hrn1"></div>
            <div class="n1">適合南美三國</div>
            <h3>影片</h3>
            <div class="hrn1"></div>
            <div class="n1">
              <div class="row">
                <div class="col-md-4">
                  <h4>天才少年Liereyy<br />印加老鷹塔vs<br />美國第一高手</h4>
                  <iframe
                    width="100%"
                    src="https://www.youtube.com/embed/ndYDHAVVRY0?list=PLBSs8pc_eAe6Mf4GRRdY_WtarywcK7zXg"
                    frameborder="0"
                    allow="autoplay; encrypted-media"
                    allowfullscreen
                  ></iframe>
                </div>
                <div class="col-md-4">
                  <h4>
                    天才少年Liereyy<br />馬雅老鷹開局vs<br />Viper裝甲弓矛
                  </h4>
                  <iframe
                    width="100%"
                    src="https://www.youtube.com/embed/XVdBZPGo-zk"
                    frameborder="0"
                    allow="autoplay; encrypted-media"
                    allowfullscreen
                  ></iframe>
                </div>
                <div class="col-md-4">
                  <h4>天才少年Liereyy<br />標準老鷹開局<br />vs Daut</h4>
                  <iframe
                    width="100%"
                    src="https://www.youtube.com/embed/bs3yD67RPak"
                    frameborder="0"
                    allow="autoplay; encrypted-media"
                    allowfullscreen
                  ></iframe>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="col-md-4">
        <table class="table-bordered" width="100%">
          <tr>
            <td class="unit" colspan="2" style="background-color:#5cb85c;">
              <div class="unititle"><nuxt-link to="">老鷹開局</nuxt-link></div>
            </td>
          </tr>
          <tr>
            <td colspan="2">
              <div class="unititle">
                <img width="45px" src="~assets/EagleScoutIcon.jpg" alt="" />
              </div>
            </td>
          </tr>
          <tr>
            <td class="unit" colspan="2" style="background-color:#5cb85c;">
              <div class="unititle2"><nuxt-link to="">戰術介紹</nuxt-link></div>
            </td>
          </tr>
          <tr>
            <td class="unit3">適合文明</td>
            <td class="unit4">南美國</td>
          </tr>
          <tr>
            <td class="unit3">時代</td>
            <td class="unit4">封建時代</td>
          </tr>
          <tr>
            <td class="unit3">威力</td>
            <td class="unit4">強</td>
          </tr>
          <tr>
            <td class="unit3">操控難易度</td>
            <td class="unit4">普</td>
          </tr>
          <tr>
            <td class="unit3">優點</td>
            <td class="unit4">弱點少，但成形慢，可銜接城堡</td>
          </tr>
          <tr>
            <td class="unit3">缺點</td>
            <td class="unit4">成形慢，小弓一多也不怕老鷹，怕圍死</td>
          </tr>
        </table>

        <div class="thanks">感謝! 此頁面由眠協助編輯</div>
      </div>
    </div>
  </div>
</template>
<script>
export default {
  layout: 'allar',
  components: {}
}
</script>
<style></style>
